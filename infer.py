...
def process_openimages_multilabel_predictions(predictions, use_trainable_labels):
  ...
  for prob, label_idx in most_probable_predictions:
    ...
    print(f'With probability {prob} network detected label: {predicted_label[1]} - (id = {predicted_label[0]})')
...
def perform_multilabel_classification_densenet(X, checkpoint_dir, use_trainable_labels):
...
def main(args):
  ...
  perform_multilabel_classification_densenet(X, args.checkpoint_dir, args.use_trainable_labels)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--path_to_test_image', 
                      type=str, 
                      required=False,
                      default='./resources/img_id_1.jpg',
