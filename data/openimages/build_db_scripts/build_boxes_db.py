...
def setup_images_db(db_cursor, table_name):
  try:
...
def setup_images_boxes_db(db_cursor, table_name, referenced_image_table_name, referenced_labels_table_name):
  try:
    db_cursor.executescript(f'''
      CREATE TABLE IF NOT EXISTS {table_name} (
        id INTEGER PRIMARY KEY NOT NULL,
...
def setup_labels_db(db_cursor, table_name):
  try:
    db_cursor.executescript(f'''
      CREATE TABLE IF NOT EXISTS {table_name} (
        id INTEGER PRIMARY KEY NOT NULL,
...
def setup_sql_schema(**kwargs):
  ...
  # training data
  setup_images_db(cursor, kwargs[ArgParseOptionsNames.train_images_table_name])
  # validation data
  setup_images_db(cursor, kwargs[ArgParseOptionsNames.val_images_table_name])
