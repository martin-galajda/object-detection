...
class ArgParseOptionNames:
    path_to_images_db = 'path_to_images_db'
    table_name_for_images = 'table_name_for_images'
    start_idx = 'start_idx'
    num_of_images_to_process = 'num_of_images_to_process'
...
if __name__ == '__main__':
    ...
    parser.add_argument(f'--{ArgParseOptionNames.path_to_images_db}',
                        type=str,
                        default=OpenImagesConstants.IMAGES_DB_PATH,
                        required=False,
                        help='Path to file containing image metadata.')

    parser.add_argument(f'--{ArgParseOptionNames.table_name_for_images}',
                        type=str,
                        required=True,
                        help='Name of table which should be appended with image bytes. (validation_images | train_images)')

    parser.add_argument(f'--{ArgParseOptionNames.start_idx}',
                        type=int,
                        default=1,
                        required=False,
                        help='Index (=id) of image to start processing from.')

    parser.add_argument(f'--{ArgParseOptionNames.num_of_images_to_process}',
                        type=int,
                        default=100,
                        required=False,
                        help='Number of images to process from the table.')

    parser.add_argument(f'--{ArgParseOptionNames.batch_process_count}',
                        type=int,
                        default=16,
                        required=False,
    ...
    parser.add_argument(f'--{ArgParseOptionNames.max_hours_to_run}',
                        type=int,
                        default=23,
                        required=False,
                        help='Maximum number of hours to perform import.')

    parser.add_argument(f'--{ArgParseOptionNames.img_size}',
                        type=int,
                        default=225,
                        required=False,
